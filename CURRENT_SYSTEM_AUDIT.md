...
1. **Apply the pending migration batch to the DATABASE** (`20260804000001` + `20260807000001`…`20260807000010`). CONFIRMED NOT applied remotely: `messages.content_encrypted`, `payments.encrypted_payload`, bank-account encrypted columns all return `does not exist` via PostgREST (2026-08-09). Effects while unapplied: push-notification subscribe (edge notify) errors until `push_subscriptions` exists, at-rest encryption not active. Apply via `npx supabase db push` (CLI already linked to `gsucsqtqtpaeuxwrykmf`; needs access token / DB password **from user**) — or paste `apply-pending-migrations.sql` into the Supabase Dashboard SQL editor (bundle includes the tracking INSERT so a later `db push` won't re-run them). **This is the next user-actionable blocker.**
2. **Release APK — RESOLVED** (2026-08-09): `android/renty-release.jks` + `android/keystore.properties` present and working. `app-release.apk` (5.6 MB, R8-minified, signed CN=Renty, SHA-256 `d17bdfd6…`) built, copied to repo root. Ready for store / distribution. The earlier "keystore missing" note was stale — the keystore and properties were on disk all along.
