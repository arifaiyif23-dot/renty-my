...
- **IC hash salt (20260820000011)**: `hash_ic_number()` previously fell back to a PUBLICLY-KNOWN salt (`r3nty_ic_salt_2026_...`, in git history) whenever the GUC was unset — and the GUC was NULL on prod, making all IC hashes trivially reversible. Fixed: a random 32-byte salt is generated and stored in `platform_settings.ic_hash_salt` (GUC preferred, then table, then RAISE — fails closed, no known fallback). Legacy hashes in `verification_requests.ic_number_hash` and `profiles.identity_number_hash` were NULLed (lossless: no app code reads them). Same pgBouncer rule as the encryption key — keep the `platform_settings` fallback.
- **Phase B security (2026-08-20)**: `rental-evidence` bucket private + signed-URL display (`useSignedUrls`); `payment-callback` per-bill `cb_token` shared-secret (constant-time, fail-closed) + billcode match; rate limits via `check_rate_limit_track` RPC on 8 abuse-prone edge fns (see `_shared/ratelimit.ts`, fail-open, 429); `cleanup-rate-limits` cron every 6h; all client-called fns (`verify-admin`, `admin-operations`, `submit-condition-report`, `complete-rental`) now `verify_jwt=true` in config.toml.
