    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-background">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">{t('paymentSuccess.verifying')}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="card-base p-8 max-w-sm w-full text-center">
        {showSuccess ? (
          <>
            ...
            <h1 className="text-xl font-bold mb-1">{t('paymentSuccess.confirmed')}</h1>
            {paymentInfo && (
              <p className="text-sm text-muted-foreground mb-1">
                RM {paymentInfo.amount.toFixed(2)} — {paymentInfo.itemTitle}
              </p>
            )}
            <p className="text-xs text-muted-foreground mb-6">{t('paymentSuccess.checkDashboard')}</p>
          ...
          <>
            ...
            <h1 className="text-xl font-bold mb-1">{t('paymentSuccess.notConfirmed')}</h1>
            <p className="text-sm text-muted-foreground mb-6">{t('paymentSuccess.tryAgainOrSupport')}</p>
            <div className="flex flex-col gap-2">
              <Button onClick={() => navigate(-1)} className="w-full rounded-lg">
                {t('paymentSuccess.retryPayment')}
              </Button>
              <Button onClick={() => navigate('/dashboard')} variant="outline" className="w-full rounded-lg">
                {t('paymentSuccess.goDashboard')}
              </Button>
              <a href="mailto:support@renty.my" className="text-sm text-primary hover:underline mt-2">
