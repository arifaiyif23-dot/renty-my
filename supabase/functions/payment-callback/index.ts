...
serve(async (req) => {
  try {
    ...
    // Shared-secret check: the bill callback URL carries ?cb_token=<token>
    // generated by create-payment and stored on the payment row. Reject any
    // callback that doesn't present the matching token (constant-time).
    const cbToken = new URL(req.url).searchParams.get('cb_token');
    if (payment.callback_token && (!cbToken || !safeEqual(cbToken, payment.callback_token))) {
      return new Response('Unauthorized', { status: 401 });
    }

    // Cross-bill confusion guard: the echoed billcode must match the bill we
    // created for this payment.
    if (payment.toyyibpay_bill_code && billCode && payment.toyyibpay_bill_code !== billCode) {
      return new Response('Bill code mismatch', { status: 401 });
    }
